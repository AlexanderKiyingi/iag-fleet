...
| POST | `/api/jmps/:id/complete` | `complete_jmp` | — (requires `active` status) |
...
| POST | `/api/requests/:id/create-jmp` | `add_jmp` | — (server fills distance/fuel/budget when possible) |
...
| Method | Path | Permission | Description |
|---|---|---|---|
| GET | `/api/vehicles/:id/track` | `view_telemetry` | GPS playback (`from`, `to`, `limit`, optional `after` RFC3339 cursor; range ≤30 d default, ≤50 k rows). Unknown vehicle → 404. Response may include `hasMore` + `nextAfter`. |
| GET | `/api/vehicles/:id/track/latest` | `view_telemetry` | Most recent ping |
| GET | `/api/vehicles/:id/telemetry/daily` | `view_telemetry` | Daily rollups (`telemetry_daily`, up to 365 d) after raw purge |
| POST | `/api/vehicles/:id/trips/detect` | `change_trip` | Infer trips from GPS in `from`/`to` window; creates `autoGenerated` rows |
...
// event: ping (single vehicle) — matches iot.Ping JSON
{ "vehicleId": "VEH-001", "ts": "2026-05-28T10:00:00Z", "lat": 0.31, "lng": 32.58,
  "speedKmh": 42, "heading": 187, "fuelLevel": 0.83, "odo": 124501.4, "ignition": true }
...
- GPS playback: ≤ **30 days** per request (override `TELEMETRY_TRACK_MAX_DAYS`), ≤ **50 k rows**. Paginate with `after` + `nextAfter` when `hasMore` is true.
