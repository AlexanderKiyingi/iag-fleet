...
func (s *Store) InsertPings(ctx context.Context, pings []Ping) (int, error) {
	...
	n, err := s.pool.CopyFrom(ctx,
		pgx.Identifier{"telemetry_pings"},
		cols,
...
func (s *Store) LatestPing(ctx context.Context, vehicleID string) (*Ping, error) {
	const q = `
        SELECT id, vehicle_id, device_id, ts, lat, lng,
               altitude, heading, speed_kmh, satellites,
               odo, fuel_level, ignition, event_id, raw
        FROM telemetry_pings WHERE vehicle_id = $1
        ORDER BY ts DESC LIMIT 1`
	row := s.pool.QueryRow(ctx, q, vehicleID)
	p, err := scanPing(row)
	if errors.Is(err, pgx.ErrNoRows) {
		return nil, nil
	}
...
func (s *Store) Track(ctx context.Context, vehicleID string, from, to time.Time, limit int) ([]Ping, error) {
	...
	rows, err := s.pool.Query(ctx, q, vehicleID, from, to, limit)
	if err != nil {
		return nil, err
	}
	defer rows.Close()
...
func (s *Store) PingsForDay(ctx context.Context, vehicleID string, day time.Time) ([]Ping, error) {
	...
	const q = `
        ...
        ORDER BY ts`
	rows, err := s.pool.Query(ctx, q, vehicleID, start, end)
	if err != nil {
		return nil, err
	}
	defer rows.Close()
...
func (s *Store) PurgeBefore(ctx context.Context, cutoff time.Time) (int64, error) {
	tag, err := s.pool.Exec(ctx, `DELETE FROM telemetry_pings WHERE ts < $1`, cutoff)
	if err != nil {
		return 0, err
	}
...
func scanPing(row rowScanner) (*Ping, error) {
	...
	var raw []byte
	err := row.Scan(
		&p.ID, &p.VehicleID, &p.DeviceID, &p.TS, &p.Lat, &p.Lng,
		&p.Altitude, &p.Heading, &p.SpeedKmh, &p.Satellites,
		&p.Odo, &p.FuelLevel, &p.Ignition, &p.EventID, &raw,
	)
	if err != nil {
