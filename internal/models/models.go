...
type Driver struct {
	...
	// --- Collected by the driver form and previously discarded (0044) ---
	NationalID string `json:"nationalId,omitempty" db:"national_id"`
	EmployeeNo string `json:"employeeNo,omitempty" db:"employee_no"`
	HireDate   string `json:"hireDate,omitempty"   db:"hire_date" dbcast:"date"`
...
type Vehicle struct {
	...
	// --- Collected by the fleet forms and previously discarded (0044) ---
	// Current cover carried on the asset, read by the service-reminder scan.
	// The renewal workflow lives on compliance_items ("PSV Insurance",
	// "Annual Inspection"); this is the snapshot, not a replacement for it.
	InsuranceProvider  string `json:"insuranceProvider,omitempty"  db:"insurance_provider"`
	InsurancePolicy    string `json:"insurancePolicy,omitempty"    db:"insurance_policy"`
	InsuranceExpiry    string `json:"insuranceExpiry,omitempty"    db:"insurance_expiry"    dbcast:"date"`
...
type Trip struct {
	...
	// --- Collected by the trip form and previously discarded (0044) ---
	Purpose    string `json:"purpose,omitempty"    db:"purpose"`
	Department string `json:"department,omitempty" db:"department"`
	// DistanceKm is what the app derives from these two. Keeping only the
	// difference loses the readings, which are what a disputed trip is settled
